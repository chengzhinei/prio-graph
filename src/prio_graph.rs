...
use crate::prio_graph::tests::TxId;
    ...
    > PrioGraph<Id, Rk, Tl, Pfn>
{
    ...
    pub fn natural_batches(
        ...
        graph.make_natural_batches_new()
    }
    ...
    pub fn make_natural_batches_new(&mut self) -> Vec<Vec<Id>> {
        // Create natural batches by manually popping without unblocking at each level.
        let mut batches = vec![];

        while !self.main_queue_new.is_empty() {
            let mut batch = Vec::new();
            while let Some(id) = self.main_queue_new.pop_front() {
                batch.push(id);
            }
    ...
    pub fn unblock(&mut self, id: &Id) -> Vec<Id> {
        ...
        for blocked_tx in edges.iter() {
            ...
            if blocked_tx_node.blocked_by_count == 0 {
                println!("main_queue push:{:?}", blocked_tx);
                self.main_queue.push(self.create_top_level_id(*blocked_tx));
                self.main_queue_new.push_back(*blocked_tx);
            }
        }
...
mod tests {
    ...
    fn test_simple_queue() {
        ...
        let (transaction_lookup_table, transaction_queue) =
            setup_test([(vec![2, 1, 3], vec![], vec![0]), ]);
        for ele in &transaction_queue.clone() {
            println!("{}", ele);
        }


        let it = create_lookup_iterator(&transaction_lookup_table, &transaction_queue);
        for (tx_id, resources) in it {
            println!("{}", tx_id)
        }
    ...
    #[test]
    fn test_simple_queue_new() {
        // Setup:
        // 3 -> 2
        // 1
        // batches: [3, 1], [2]
        let (transaction_lookup_table, transaction_queue) =
            setup_test([(vec![5,4], vec![], vec![0]), (vec![1,2,3], vec![], vec![1])]);
        // for ele in &transaction_queue.clone() {
        //     println!("{}", ele);
        // }
        ...
        assert_eq!(batches, [vec![5, 3], vec![4, 2], vec![1]]);
    }
    ...
    fn test_multiple_separate_queues() {
        ...
        assert_eq!(batches, [vec![8, 5, 6], vec![7 4], vec![3, 2], vec![1]]);
    }
